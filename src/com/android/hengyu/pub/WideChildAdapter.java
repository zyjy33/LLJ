...
public class WideChildAdapter extends BaseAdapter {
	...
	public View getView(final int position, View view, ViewGroup arg2) {
		...
		if (view == null || view.getTag(R.drawable.ic_launcher + position) == null) {
			...
			RelativeLayout el_quanbu  = (RelativeLayout) view.findViewById(R.id.el_quanbu);
//			TextView tv = BaseViewHolder.get(view, R.id.tv_item);
//			ImageView iv = BaseViewHolder.get(view, R.id.iv_item);
//			RelativeLayout el_quanbu = BaseViewHolder.get(view, R.id.el_quanbu);
			tv.setText(items.get(position).getTitle()); 
			aQuery.id(iv).image(RealmName.REALM_NAME_HTTP + items.get(position).getIcon_url());
			el_quanbu.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					try {
			      	...
			      	System.out.println("=getTitle============"+items.get(position).getTitle());
			      	
	            Intent intent= new Intent(context,SymptomsJieShaoActivity.class);
//	            ArrayList<GuigellBean>  bean =  items.get(position).getList();
...
//		        intent.putExtras(bundle);
	            ...
	            intent.putExtra("proposal",items.get(position).proposal);

...
public class ZhongAnYlAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup patent) {
		...
		TextView tv_letter = (TextView) convertView.findViewById(R.id.tagname);
		LinearLayout addview= (LinearLayout) convertView.findViewById(R.id.addview);
		ImageView iv_zhuti = (ImageView) convertView.findViewById(R.id.iv_zhuti);
...
//			tv_letter.setBackgroundColor(getResources().getColor(R.color.white));
		...
		for (int i = 0; i <list.get(position).getList().size() ; i++) {
			...
			View vi = LayoutInflater.from(context).inflate(R.layout.zayl_telie,null);//zayl_item

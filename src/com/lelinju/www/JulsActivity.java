...
public class JulsActivity extends BaseActivity {
	...
	private void init() {
		...
		lglotteryLogAdapter = new JuLogAdapter(getApplicationContext(),
				lists, imageLoader);
	...
	private void connect() {
		...
		AsyncHttp.post(RealmName.REALM_NAME+"/mi/LotteryGame.ashx?act=GetUserLuckDrawHistoryList", params, new AsyncHttpResponseHandler() {
			public void onStart() {
			};
			...
			public void onSuccess(int arg0, String arg1) {
				...
				try {
					...
					for (int i = 0; i < len; i++) {
						...
						lglottery_Log.setId(object.getString("EventName"));

						try {
							lglottery_Log.setTime(formatter.format(formatter
									.parse(object.getString("LuckDrawTime")
											.replaceAll("/", "-"))));
...
//						if (sta.equals("1")) {
							lglottery_Log.setName(object.getString("PrizeName"));

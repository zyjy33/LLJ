...
//		spPreferences = MyShoppingCartllActivity.this.getSharedPreferences("longuserset", Context.MODE_PRIVATE);
	...
	private void ininate() {
		...
		btn_register.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				user_id = spPreferences.getString("user_id", "");
				if (user_id.equals("")) {
					Intent intentll = new Intent(MyShoppingCartllActivity.this,UserLoginActivity.class);
					startActivity(intentll);
				} else {
						Intent intentll = new Intent(MyShoppingCartllActivity.this,NewWare.class);
						startActivity(intentll);
				}
			}
		});
			...
			case 2:
				try {
					...
					if (shopping_id>0) {
						...
						Toast.makeText(MyShoppingCartllActivity.this,"请勾选要下单的商品", 200).show();
					}
					} catch (Exception e) {
						// TODO: handle exception
						e.printStackTrace();
...
//							Toast.makeText(getActivity(), "购物车商品", 200).show();
							}else {
								progress.CloseProgress();
								adapter = new MyShopingCartllAdapter(list_ll, MyShoppingCartllActivity.this, handler);
								list_shop_cart.setAdapter(adapter);
								...
								ll_xianshi.setVisibility(View.VISIBLE);
//								Toast.makeText(getActivity(), "购物车暂无商品", 200).show();
							}
...
//									Toast.makeText(getActivity(), info, 200).show();
								    System.out.println("拼接之后total_cll---------------"+total_cll);
...
//									finish();
									...
									progress.CloseProgress();
									Toast.makeText(MyShoppingCartllActivity.this, info, 200).show();
								}
							} catch (JSONException e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
...
//					handler.sendMessage(message);
					Toast.makeText(MyShoppingCartllActivity.this, "删除成功", 200).show();
//					List<String> list_cart_id = MyShopingCartllAdapter.list_cart_id;
//					 List<String> list_goods_id = MyShopingCartllAdapter.list_goods_id;
//					 List<Integer> list_quantity = MyShopingCartllAdapter.list_quantity;
					 System.out.println("list_cart_id================"+list_cart_id.size());

...
//		img_title_login.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.get_yz:
			...
			if (phone.equals("")) {
				Toast.makeText(UserForgotPasswordActivity.this, "手机号码不能为空", 200).show();
			}else if (phone.length() < 11 ) {
				Toast.makeText(UserForgotPasswordActivity.this, "手机号码少于11位", 200).show();
			}else {
				if(Validator.isMobile(phone)){
					
				strUrl = RealmName.REALM_NAME_LL+"/user_passmis_smscode?mobile="+phone+"";
							...
							public void onSuccess(int arg0, String arg1) {
								...
								try {
									...
									if (result.equals("y")) {
									    ...
									    Toast.makeText(UserForgotPasswordActivity.this, info, 200).show();
//										handler.sendEmptyMessage(3);
				...
				Toast.makeText(UserForgotPasswordActivity.this, "手机号码不正确", 200).show();
			}
			}
...
//			pwdagain = userpwdagain.getText().toString().trim();
			...
			if (phone.equals("")) {
				...
				Toast.makeText(UserForgotPasswordActivity.this, "验证码不能为空", 200).show();
			}
//			else if (!yz.equals(yanzhengma)) {
//				Toast.makeText(UserRegisterActivity.this, "验证码不正确", 200).show();
//			}
//			else if (yz.length() < 6 ) {
//				Toast.makeText(UserForgotPasswordActivity.this, "验证码少于六位", 200).show();
//			}
			else if (pwd.equals("")) {
				Toast.makeText(UserForgotPasswordActivity.this, "密码不能为空", 200).show();
			} else if (pwd.length()<8) {
				Toast.makeText(UserForgotPasswordActivity.this, "密码不得小于8位", 200).show();
			} else if (!(userpwd.getText().toString().length()<20&&userpwd.getText().toString().length()>=8)) {
				Toast.makeText(UserForgotPasswordActivity.this, "密码在8-20位之间", 200).show();
			}else {
				try {
...
//								System.out.println("=================13==" + type);
								strUrl = RealmName.REALM_NAME_LL+"/mobile_update_password?user_name="+phone+"&mobile="+phone+"&code="+yz+"&newpassword="+pwd+"&type="+type+"";

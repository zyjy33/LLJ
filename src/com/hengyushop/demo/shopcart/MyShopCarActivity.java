		...
		//购物车无商品去逛逛
				btn_register.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View arg0) {
						...
						user_id = spPreferences.getString("user_id", "");
						if (user_id.equals("")) {
							Intent intentll = new Intent(MyShopCarActivity.this,UserLoginActivity.class);
							startActivity(intentll);
						} else {
								Intent intentll = new Intent(MyShopCarActivity.this,NewWare.class);
								startActivity(intentll);
						}
					}
				});
	...
	private class LoadDataTask extends AsyncTask<Params, Void, Result> {
		...
		protected void onPostExecute(Result result) {
			...
			if (result.op == INITIALIZE) {
				...
				Toast.makeText(MyShopCarActivity.this, "添加成功！",Toast.LENGTH_SHORT).show();
			}
	...
	private class ListAdapter extends BaseAdapter implements
		...
		public View getView(final int position, View convertView,
			...
			if (view == null) {
				...
				view = LayoutInflater.from(MyShopCarActivity.this).inflate(R.layout.cart_list_item, null);
		...
		private class ReduceOnClick implements OnClickListener {
			...
			public void onClick(View arg0) {
				...
				if (!TextUtils.isEmpty(numberStr)) {
					int number = Integer.parseInt(numberStr);
					if (number == 1) {
						 Toast.makeText(MyShopCarActivity.this, "不能往下减少了", 200).show();
					} else {
						int currentNum = number - 1;
						// 设置列表
						shopcartEntity.setQuantity(currentNum);
...
//					}
					if (list_size.size() == 0) {
						Toast.makeText(MyShopCarActivity.this, "请选择要删除的商品", 200).show();
					}else {
...
//											    	 	if (mListData.size() >= list_size.size()) {
//											    	 		getgouwuche();
//														}
...
//								str3 = "";//先清空
						...
						String zhou = str1+"/"+str2+"/"+str3;
						 if (str1.equals("")) {
							 Toast.makeText(MyShopCarActivity.this, "请选择要支付的商品", 200).show();
//								mListAdapter.notifyDataSetChanged();
						 }else {
...
//									Toast.makeText(MyShopCarActivity.this, info, 200).show();
									Intent intent=new Intent(MyShopCarActivity.this, MyOrderConfrimActivity.class);
